        ...
        <div className="grid gap-6 lg:grid-cols-[320px_1fr] xl:grid-cols-[360px_1fr] lg:gap-8 items-center mb-10 md:mb-12">
          ...
          <div className="space-y-0">
            {steps.map((step, index) => (
              ...
              >
                ...
                <div className="flex flex-col items-center">
                  <div className="flex-shrink-0 w-9 h-9 sm:w-10 sm:h-10 rounded-full flex items-center justify-center font-semibold text-sm sm:text-base border-2 border-border-strong text-foreground/80 bg-background">
                    {step.number}
                  </div>
                  {/* Dotted line connector (not on last step) */}
                  {index < steps.length - 1 && (
                    <div className="w-px h-8 sm:h-10 border-l-2 border-dashed border-border-strong mt-2" />
                  )}
                </div>

                {/* Step Content */}
                <div className="flex-1 min-w-0 pt-1.5">
                  <h3 className="text-sm sm:text-base font-semibold mb-0.5 text-foreground">
                    {step.title}
                  </h3>
            ...
            <div className="pt-3 pl-3 sm:pl-4">
              <a
                ...
                className="inline-flex items-center justify-center rounded-md bg-foreground px-5 sm:px-6 py-2 sm:py-2.5 text-sm font-medium text-background transition-colors cta-hover-gradient"
              >
                {signupLabel}
              </a>
            </div>
          ...
          <div className="order-first lg:order-last">
            <div className="relative aspect-video w-full rounded-xl overflow-hidden ">
              ...
              {/* Decorative Elements */}
              <div className="absolute -bottom-4 -right-4 w-24 h-24 bg-gradient-to-br from-primary/20 to-black/20 rounded-full blur-2xl pointer-events-none" />
              <div className="absolute -top-4 -left-4 w-32 h-32 bg-gradient-to-br from-primary/20 to-black/20 rounded-full blur-2xl pointer-events-none" />
            </div>
          </div>
        </div>
      </div>
