...
function HumanLikeVoicesIllustration() {
  ...
  const voices = [
    { name: "Caleb", type: "Trustworthy. Great for support & sales.", gradient: "from-background via-amber-400 to-orange-500", selected: true },
    { name: "Jack John", type: "Engaging and positive for support.", gradient: "from-background via-blue-400 to-indigo-500" },
    { name: "Joseph", type: "Professional and patient. For IVR.", gradient: "from-background via-cyan-400 to-blue-500" },
    { name: "Kyle", type: "Confident. Great for storytelling.", gradient: "from-background via-purple-400 to-pink-500" },
    { name: "Sarah", type: "Warm and friendly. Ideal for sales.", gradient: "from-background via-rose-400 to-red-500" },
    { name: "Emma", type: "Clear and articulate. For support.", gradient: "from-background via-emerald-400 to-teal-500" },
  ];
        ...
        <div className="bg-background rounded-xl shadow-sm border border-border overflow-hidden h-full flex flex-col">
          ...
          <div className="p-2.5 sm:p-3 border-b border-border">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div className="flex-1">
                <p className="text-xs sm:text-sm font-semibold text-foreground">Voice Configuration</p>
                <p className="text-[10px] sm:text-xs text-muted-foreground">Natural-sounding AI voices</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 p-2.5 sm:p-3 flex flex-col sm:flex-row gap-2 sm:gap-2.5 min-h-0">
            ...
            <div className="sm:w-[140px] md:w-[160px] flex flex-col min-h-0 flex-shrink-0">
              <p className="text-[10px] sm:text-xs text-muted-foreground mb-1.5 font-medium font-mono-ui uppercase tracking-[0.1em]">Language</p>
              <div className="flex-1 bg-surface rounded-lg border border-border overflow-hidden flex flex-col min-h-0">
                {/* Selected value */}
                <div className="flex items-center gap-1.5 px-2 sm:px-2.5 py-1.5 sm:py-2 border-b border-border bg-background">
                  <span className="text-xs sm:text-sm">🇺🇸</span>
                  <span className="text-[11px] sm:text-xs text-foreground font-medium truncate">English (US)</span>
                  <svg className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-muted-foreground ml-auto flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  {languages.map((lang, i) => (
                    <div
                      ...
                      className={`flex items-center gap-1.5 px-2 sm:px-2.5 py-1 sm:py-1.5 cursor-pointer transition-colors ${
                        lang.selected ? 'bg-primary/5' : 'hover:bg-muted'
                      }`}
                    >
                      <span className="text-[11px] sm:text-xs">{lang.flag}</span>
                      <span className={`text-[11px] sm:text-xs truncate ${lang.selected ? 'text-foreground font-medium' : 'text-muted-foreground'}`}>
                        {lang.name}
                      </span>
                      {lang.selected && (
                        <svg className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-primary ml-auto flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="flex-1 flex flex-col min-h-0">
              <p className="text-[10px] sm:text-xs text-muted-foreground mb-1.5 font-medium font-mono-ui uppercase tracking-[0.1em]">Voice</p>
              <div className="flex-1 bg-surface rounded-lg border border-border overflow-hidden flex flex-col min-h-0">
                {/* Selected value */}
                <div className="flex items-center gap-1.5 px-2 sm:px-2.5 py-1.5 sm:py-2 border-b border-border bg-background">
                  <div className="w-4 h-4 sm:w-5 sm:h-5 rounded-full bg-gradient-to-br from-background via-amber-400 to-orange-500 flex items-center justify-center shadow-sm flex-shrink-0">
                  ...
                  <span className="text-[11px] sm:text-xs text-foreground font-medium">Caleb</span>
                  <svg className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-muted-foreground ml-auto flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 15l7-7 7 7" />
                  </svg>
                </div>
                ...
                <div className="flex-1 overflow-hidden">
                  {voices.map((voice, i) => (
                    <div
                      key={i}
                      className={`flex items-center gap-1.5 px-2 sm:px-2.5 py-1 sm:py-1.5 cursor-pointer transition-colors ${
                        voice.selected ? 'bg-primary/5' : 'hover:bg-muted'
                      }`}
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className={`text-[11px] sm:text-xs ${voice.selected ? 'text-foreground font-medium' : 'text-foreground/80'}`}>
                          {voice.name}
                        </p>
                        <p className="text-[9px] sm:text-[11px] text-muted-foreground truncate">{voice.type}</p>
                      </div>
                      {voice.selected && (
        ...
        <div className="bg-background rounded-xl shadow-sm border border-border h-full flex flex-col overflow-hidden">
          ...
          <div className="flex-1 p-3 sm:p-4 space-y-4 sm:space-y-5 min-h-0">
            ...
            <div>
              ...
              <div className="h-1.5 sm:h-2 rounded-full bg-muted relative group cursor-pointer">
                <div className="absolute left-0 top-0 h-1.5 sm:h-2 w-3/4 rounded-full bg-gradient-to-r from-gray-400 to-primary transition-all duration-300" />
                <div className="absolute left-3/4 top-1/2 -translate-y-1/2 -translate-x-1/2 w-3.5 h-3.5 sm:w-4 sm:h-4 rounded-full bg-background border-2 border-primary shadow-sm transition-transform duration-200 group-hover:scale-110" />
              </div>
...
function SpeechRecognitionIllustration() {
  ...
  return (
    <div className="relative h-full w-full rounded-xl overflow-hidden" style={{ backgroundImage: 'radial-gradient(circle, hsl(var(--border)) 1px, transparent 1px)', backgroundSize: '16px 16px', backgroundPosition: 'center center' }}>
      ...
      <div className="relative h-full w-full p-2.5 sm:p-3 md:p-4">
        {/* Main card - light theme with grey accents */}
        <div className="bg-background rounded-xl shadow-sm border border-border overflow-hidden h-full flex flex-col p-2.5 sm:p-3">
          {/* Header */}
          <div className="flex items-center justify-between mb-2 sm:mb-2.5">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div className="flex-1">
                <p className="text-xs sm:text-sm font-semibold text-foreground">Speech Recognition</p>
                <p className="text-[10px] sm:text-xs text-muted-foreground">Deepgram & OpenAI Whisper</p>
              </div>
            </div>
            <div className="flex items-center gap-1 bg-primary/10 px-1.5 py-0.5 rounded-full border border-primary/20">
              <span className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse" />
              <span className="text-[10px] sm:text-xs text-primary font-medium">Live</span>
            </div>
          </div>

          {/* Live waveform with animation */}
          <div className="rounded-lg bg-surface border border-border p-2 mb-2 sm:mb-2.5">
            <div className="flex items-center gap-1.5 mb-1.5">
              <div className="w-2 h-2 rounded-full bg-red-500 animate-pulse" />
              <span className="text-[9px] sm:text-[10px] text-muted-foreground font-medium font-mono-ui uppercase tracking-[0.1em]">Recording</span>
            </div>
            <div className="flex items-center justify-center gap-[2px] sm:gap-[3px] h-8 sm:h-10">
              {waveHeights.map((height, i) => (
                <div
                  key={i}
                  className="wave-bar w-[2px] sm:w-[3px] rounded-full bg-primary"
                  style={{
                    height: `${height}px`,
          ...
          <div className="flex-1 rounded-lg bg-surface border border-border p-2 overflow-hidden min-h-0">
            <div className="flex items-center justify-between mb-1.5">
              <p className="text-[10px] sm:text-xs text-muted-foreground font-medium">Real-time transcription</p>
              <span className="text-[9px] sm:text-[10px] text-muted-foreground">00:32</span>
            </div>
            <div className="space-y-1.5 sm:space-y-2">
              {/* Customer message 1 */}
              <div className="flex items-start gap-1.5">
                <div className="w-4 h-4 sm:w-5 sm:h-5 rounded-full bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center flex-shrink-0 mt-0.5">
                  <svg className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-emerald-600 dark:text-emerald-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                  </svg>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-[9px] sm:text-[10px] text-emerald-600 dark:text-emerald-300 font-medium mb-0.5">Customer</p>
                  <p className="text-[11px] sm:text-xs text-foreground/80 leading-relaxed">"Hi, I'd like to check my order status for order #45892."</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-1.5">
                <div className="w-4 h-4 sm:w-5 sm:h-5 rounded-full bg-primary/10 flex items-center justify-center flex-shrink-0 mt-0.5">
                  <span className="text-[8px] sm:text-[9px] text-primary">✦</span>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-[9px] sm:text-[10px] text-primary font-medium mb-0.5">AI Agent</p>
                  <p className="text-[11px] sm:text-xs text-foreground/80 leading-relaxed">"Of course! Let me look that up for you right away."</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-1.5">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[9px] sm:text-[10px] text-emerald-600 dark:text-emerald-300 font-medium mb-0.5">Customer</p>
                  <p className="text-[11px] sm:text-xs text-foreground/80 leading-relaxed">"It was supposed to arrive yesterday..."</p>
                </div>
              </div>
          ...
          {/* Stats */}
          <div className="flex items-center justify-between mt-2 sm:mt-2.5 pt-2 border-t border-border">
            <div className="text-center">
              <p className="text-xs sm:text-sm font-semibold text-primary">98.5%</p>
              <p className="text-[9px] sm:text-[10px] text-muted-foreground">Accuracy</p>
            </div>
            <div className="text-center">
              <p className="text-xs sm:text-sm font-semibold text-foreground">&lt;200ms</p>
              <p className="text-[9px] sm:text-[10px] text-muted-foreground">Latency</p>
            </div>
            <div className="text-center">
              <p className="text-xs sm:text-sm font-semibold text-foreground">50+</p>
              <p className="text-[9px] sm:text-[10px] text-muted-foreground">Languages</p>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="relative h-full w-full rounded-xl overflow-hidden" style={{ backgroundImage: 'radial-gradient(circle, hsl(var(--border)) 1px, transparent 1px)', backgroundSize: '16px 16px', backgroundPosition: 'center center' }}>
      <div className="relative h-full w-full p-3 sm:p-4 md:p-6 flex flex-col justify-center gap-3 sm:gap-4">
        ...
        <div className="bg-background rounded-xl shadow-sm border border-border p-3 sm:p-4">
          {/* Order header */}
          <div className="flex items-center justify-between text-[10px] sm:text-xs text-muted-foreground mb-3">
            ...
            <span className="text-emerald-600 dark:text-emerald-300 font-medium">Payment received</span>
          </div>
          ...
          <div className="flex items-start gap-3">
            ...
            {/* Product info */}
            <div className="flex-1 min-w-0">
              <h4 className="text-base sm:text-lg font-medium text-muted-foreground mb-1">Cloudscape T-shirt</h4>
              <p className="text-xs sm:text-sm text-muted-foreground mb-2">Size: L</p>
              <div className="flex items-center gap-3">
                <span className="text-[11px] sm:text-xs text-primary">Track Order</span>
                <span className="text-[11px] sm:text-xs text-primary">Invoice</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-background rounded-xl shadow-sm border border-border p-3 sm:p-4">
          ...
          <div className="flex items-start gap-2.5 sm:gap-3">
            ...
            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-gradient-to-br from-primary to-black flex items-center justify-center flex-shrink-0 shadow-sm">
              <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-gradient-to-br from-primary/80 to-primary/80 flex items-center justify-center border border-white/30">
            ...
            {/* Message content */}
            <div className="flex-1 min-w-0">
              <p className="text-[10px] sm:text-xs text-muted-foreground mb-1">TrendFit AI Agent</p>
...
function GuardrailsIllustration() {
  return (
    <div className="relative h-full w-full rounded-xl overflow-hidden" style={{ backgroundImage: 'radial-gradient(circle, hsl(var(--border)) 1px, transparent 1px)', backgroundSize: '16px 16px', backgroundPosition: 'center center' }}>
      <div className="relative h-full w-full p-2.5 sm:p-3 md:p-4">
        {/* Main card - light theme with grey accents */}
        <div className="bg-background rounded-xl shadow-sm border border-border overflow-hidden h-full flex flex-col p-3 sm:p-4">
          {/* Header */}
          <div className="flex items-center gap-2.5 sm:gap-3 mb-3 sm:mb-4">
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-gradient-to-br from-gray-700 to-gray-900 flex items-center justify-center shadow-sm">
              <svg className="w-4 h-4 sm:w-5 sm:h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
              </svg>
            </div>
            <div className="flex-1">
              <p className="text-sm sm:text-base font-semibold text-foreground">AI Guardrails</p>
              <p className="text-xs sm:text-sm text-primary">All checks passing</p>
            </div>
          </div>
          ...
          <div className="space-y-1.5 sm:space-y-2 flex-1 min-h-0 overflow-hidden">
            {[
              ...
              <div key={i} className="flex items-center gap-2 p-2 sm:p-2.5 rounded-lg bg-surface border border-border">
                {/* Checkbox */}
                ...
                <span className="text-[11px] sm:text-xs text-foreground flex-1 font-medium">{item.rule}</span>
                <span className="text-[9px] sm:text-[10px] text-primary bg-primary/10 px-1.5 py-0.5 rounded font-medium">Active</span>
              </div>
            ))}
          </div>
          ...
          <div className="flex items-center justify-between mt-3 sm:mt-4 pt-3 border-t border-border">
            <div className="text-center">
              <p className="text-sm sm:text-base font-semibold text-foreground">0</p>
              <p className="text-[10px] sm:text-xs text-muted-foreground">Violations</p>
            </div>
            <div className="text-center">
              <p className="text-sm sm:text-base font-semibold text-foreground">2,847</p>
              <p className="text-[10px] sm:text-xs text-muted-foreground">Monitored</p>
            </div>
            <div className="text-center">
              <p className="text-sm sm:text-base font-semibold text-primary">100%</p>
              <p className="text-[10px] sm:text-xs text-muted-foreground">Compliance</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="bg-background rounded-xl shadow-sm border border-border overflow-hidden h-full flex flex-col p-3 sm:p-4">
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-gradient-to-br from-primary to-black flex items-center justify-center">
                <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
                </svg>
              </div>
              <div>
                <p className="text-sm sm:text-base font-semibold text-foreground">Voice AI Dashboard</p>
                <p className="text-[10px] sm:text-xs text-muted-foreground">Real-time analytics</p>
              </div>
            </div>
            <div className="flex items-center gap-1 bg-green-50 dark:bg-green-900/30 px-2 py-1 rounded-full border border-green-200 dark:border-green-900/50">
              <span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" />
              <span className="text-[10px] sm:text-xs text-green-700 dark:text-green-300 font-medium">Live</span>
            </div>
          </div>
          ...
          <div className="flex-1 grid grid-cols-5 gap-3 min-h-0">
            ...
            <div className="col-span-3 flex flex-col">
              <div className="flex items-center justify-between mb-2">
                <p className="text-[10px] sm:text-xs text-muted-foreground font-medium">Call Volume (24h)</p>
                <div className="flex items-center gap-1">
                  <span className="text-[10px] text-green-600 dark:text-green-300 font-medium">+12%</span>
              ...
              <div className="flex-1 bg-surface/50 rounded-lg p-2 relative">
                {/* SVG Area Chart */}
                ...
                {/* X-axis labels */}
                <div className="absolute bottom-0.5 left-1 right-1 flex justify-between text-[7px] text-muted-foreground">
                  <span>12am</span>
                  <span>6am</span>
                  <span>12pm</span>
                  <span>6pm</span>
            ...
            <div className="col-span-2 flex flex-col gap-2">
              ...
              <div className="bg-surface rounded-lg p-2 flex items-center gap-2">
                <div className="relative w-10 h-10 sm:w-12 sm:h-12">
                  ...
                  <span className="absolute inset-0 flex items-center justify-center text-[10px] sm:text-xs font-bold text-foreground">94%</span>
                </div>
                <div>
                  <p className="text-xs sm:text-sm font-semibold text-foreground">Resolution</p>
                  <p className="text-[9px] sm:text-[10px] text-muted-foreground">1,247 resolved</p>
                </div>
              </div>
              ...
              <div className="bg-primary/5 rounded-lg p-2 border border-primary/10">
                ...
                <p className="text-[9px] sm:text-[10px] text-muted-foreground">Concurrent Calls</p>
              </div>

              {/* Call Type Bars */}
              <div className="bg-surface rounded-lg p-2 flex-1">
                <p className="text-[9px] text-muted-foreground mb-1.5">By Type</p>
                <div className="space-y-1.5">
                  {[
                    ...
                    <div key={item.type} className="flex items-center gap-1.5">
                      <span className="text-[8px] sm:text-[9px] text-muted-foreground w-10">{item.type}</span>
                      <div className="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
                        <div className="h-full rounded-full" style={{ width: `${item.value}%`, backgroundColor: item.color }} />
                      </div>
                      <span className="text-[8px] text-muted-foreground w-6">{item.value}%</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="flex items-center justify-between mt-3 pt-2 border-t border-border">
            ...
            <span className="text-[10px] sm:text-xs text-muted-foreground">Updated 2s ago</span>
          </div>
        </div>
      </div>
    </div>
