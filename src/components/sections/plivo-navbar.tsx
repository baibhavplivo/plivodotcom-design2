...
const SipIcon = ({ className }: { className?: string }) => (
  <svg className={className} viewBox="0 0 24 24" fill="currentColor">
    ...
    <text x="12" y="15" textAnchor="middle" fontSize="8" fontWeight="bold" fill="currentColor">SIP</text>
  </svg>
);
...
export const PlivoNavbar = ({ currentPage }: { currentPage: string }) => {
  ...
  return (
    <header
      className={cn(
        "fixed top-0 left-0 right-0 z-50 transition-all duration-200 border-b border-gray-100",
        isScrolled
                  ...
                  >
                    {item.sections?.map((section) => (
                      <div key={section.title} className="mb-4">
                        <h3 className="mb-2 pl-4 text-[12px] font-semibold text-[#4f5aff]">
                          {section.title}
                        </h3>
                        {section.items.map((subItem) => (
                          <a
                          ...
                          >
                            {subItem.icon && iconMap[subItem.icon] && (
                              <span className="text-[#4f5aff] mt-0.5">
                                {iconMap[subItem.icon]}
                              </span>
                            )}
                            <div>
