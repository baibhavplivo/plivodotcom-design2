  ...
  if (inline) {
    return (
      <div className="border-t border-border pt-3">
        <h4 className="font-semibold text-xs text-foreground mb-2">{title}</h4>
        <div>{children}</div>
      </div>
    );
  }
  ...
  if (!rates) {
    return (
      <div className="text-center py-8">
        <p className="text-sm text-muted-foreground">Detailed voice pricing for {countryName} is available on request.</p>
        <a href="/contact/sales/" className="mt-3 inline-block text-sm font-medium text-primary hover:underline">Contact sales for rates</a>
      </div>
    );
  }
        ...
        <table className="w-full">
          ...
          <tbody className="text-xs">
            {voiceRows.map(([label, outbound, inbound]) => (
              <tr key={label} className="border-t border-dashed border-border">
                <td className="py-2.5 font-medium text-foreground">{label}</td>
                <td className="py-2.5 text-muted-foreground">{outbound}</td>
                <td className="py-2.5 text-muted-foreground">{inbound}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <table className="w-full">
          ...
          <tbody className="text-xs">
            {addOnRows.map(([service, price]) => (
              <tr key={service} className="border-t border-dashed border-border">
                <td className="py-2.5 font-medium text-foreground">{service}</td>
                <td className="py-2.5 text-muted-foreground">{price}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
function SMSPricingContent({ selectedCountry, inline }: { selectedCountry: string; inline?: boolean }) {
  ...
  if (smsRows.length === 0) {
    return (
      <div className="text-center py-8">
        <p className="text-sm text-muted-foreground">Detailed SMS pricing for {countryName} is available on request.</p>
        <a href="/contact/sales/" className="mt-3 inline-block text-sm font-medium text-primary hover:underline">Contact sales for rates</a>
      </div>
    );
  }
    ...
    <>
      <ModalTableSection inline={inline} title="SMS messaging">
        <table className="w-full">
          ...
          <tbody className="text-xs">
            {smsRows.map((row) => (
              <tr key={row.label} className="border-t border-dashed border-border">
                <td className="py-2.5 font-medium text-foreground">{row.label}</td>
                <td className="py-2.5 text-muted-foreground">{row.outbound}</td>
                <td className="py-2.5 text-muted-foreground">{row.inbound}</td>
              </tr>
            ))}
          </tbody>
        </table>
          ...
          <table className="w-full">
            ...
            <tbody className="text-xs">
              {SMS_RCS_RATES.map((row) => (
                <tr key={row.type} className="border-t border-dashed border-border">
                  <td className="py-2.5 font-medium text-foreground">{row.type}</td>
                  <td className="py-2.5 text-muted-foreground">{row.outbound}</td>
                  <td className="py-2.5 text-muted-foreground">{row.inbound}</td>
                </tr>
              ))}
...
function WhatsAppPricingContent({ selectedCountry, inline }: { selectedCountry: string; inline?: boolean }) {
  ...
  if (!chatRates && !callRates) {
    return (
      <div className="text-center py-8">
        <p className="text-sm text-muted-foreground">Detailed WhatsApp pricing for {countryName} is available on request.</p>
        <a href="/contact/sales/" className="mt-3 inline-block text-sm font-medium text-primary hover:underline">Contact sales for rates</a>
      </div>
    );
  }
          ...
          <table className="w-full">
            ...
            <tbody className="text-xs">
              <tr className="border-t border-dashed border-border">
                <td className="py-2.5 font-medium text-foreground">Marketing</td>
                <td className="py-2.5 text-muted-foreground">{formatChatRate(chatRates.marketing, chatRates.currency)}</td>
              </tr>
              <tr className="border-t border-dashed border-border">
                <td className="py-2.5 font-medium text-foreground">Utility</td>
                <td className="py-2.5 text-muted-foreground">{formatChatRate(chatRates.utility, chatRates.currency)}</td>
              </tr>
              <tr className="border-t border-dashed border-border">
                <td className="py-2.5 font-medium text-foreground">Authentication</td>
                <td className="py-2.5 text-muted-foreground">{formatChatRate(chatRates.authentication, chatRates.currency)}</td>
              </tr>
              {chatRates.authenticationIntl > 0 && (
                <tr className="border-t border-dashed border-border">
                  <td className="py-2.5 font-medium text-foreground">Authentication (international)</td>
                  <td className="py-2.5 text-muted-foreground">{formatChatRate(chatRates.authenticationIntl, chatRates.currency)}</td>
                </tr>
              )}
              <tr className="border-t border-dashed border-border">
                <td className="py-2.5 font-medium text-foreground">Service</td>
      ...
      {callRates && (
        <ModalTableSection inline={inline} title="WhatsApp voice calls">
          <table className="w-full">
            ...
            <tbody className="text-xs">
              <tr className="border-t border-dashed border-border">
                <td className="py-2.5 font-medium text-foreground">Inbound</td>
                <td className="py-2.5 text-muted-foreground">{formatWhatsAppCallRate(callRates.inbound)}</td>
              </tr>
              <tr className="border-t border-dashed border-border">
                <td className="py-2.5 font-medium text-foreground">Outbound</td>
                <td className="py-2.5 text-muted-foreground">{formatWhatsAppCallRate(callRates.outbound)}</td>
              </tr>
              <tr className="border-t border-dashed border-border">
                <td className="py-2.5 font-medium text-foreground">Audio streaming & noise cancellation</td>
                <td className="py-2.5 text-muted-foreground">Included</td>
              </tr>
            </tbody>
          </table>
        </ModalTableSection>
    ...
    <>
      <ModalTableSection inline={inline} title="AI agent usage">
        <table className="w-full">
          ...
          <tbody className="text-xs">
            <tr className="border-t border-dashed border-border">
              <td className="py-2.5 font-medium text-foreground">Voice AI agent</td>
              <td className="py-2.5 text-muted-foreground">{aiRates.voice}</td>
            </tr>
            <tr className="border-t border-dashed border-border">
              <td className="py-2.5 font-medium text-foreground">AI text conversations</td>
              <td className="py-2.5 text-muted-foreground">{aiRates.text}</td>
            </tr>
          </tbody>
        </table>
        ...
        <table className="w-full">
          ...
          <tbody className="text-xs">
            {[
              ...
              <tr key={feature} className="border-t border-dashed border-border">
                <td className="py-2.5 font-medium text-foreground">{feature}</td>
                <td className="py-2.5 text-muted-foreground">{price}</td>
              </tr>
            ))}
          </tbody>
  ...
  return (
    <>
      ...
      <section className="relative w-full overflow-hidden bg-background border-t border-border">
        <div className="pointer-events-none absolute inset-0 dev-grid-bg opacity-[0.3] [mask-image:linear-gradient(to_bottom,black_30%,transparent_95%)]" />
        <div className="relative container mx-auto max-w-7xl px-4 sm:px-6 pt-10 pb-10 sm:pt-14 sm:pb-14 lg:pt-20 lg:pb-16">
          <div className="flex items-center gap-3 font-mono-ui text-[11px] uppercase tracking-[0.12em] text-muted-foreground">
            ...
            <span>pay as you go · no lock-in</span>
          </div>
          <div className="mt-8 text-center">
          <h1 className="font-sora text-[2rem] font-normal leading-[1.04] tracking-[-0.035em] text-foreground sm:text-[2.5rem] md:text-[3rem]">
            Start now with <span className="text-primary">{selectedCountry === "IN" ? "₹1,000" : "$10"}</span> free credits
          </h1>
          ...
          <div className="mt-5 flex justify-center">
            <div className="relative" ref={mainDropdownRef}>
              <button
                ...
                className="flex w-full max-w-[256px] items-center justify-between rounded-lg border border-border bg-background px-4 py-2.5 text-sm font-medium text-foreground transition-colors hover:bg-surface"
              >
              ...
              {isDropdownOpen && (
                <div className="absolute left-0 right-0 sm:left-1/2 sm:-translate-x-1/2 sm:w-64 top-full mt-1 z-50 max-h-72 overflow-hidden flex flex-col rounded-lg border border-border bg-background shadow-sm">
                    ...
                    {mainFilteredCountries.map((c) => (
                      <button
                        ...
                        className={`flex w-full items-center gap-2 px-3 py-2 text-left text-sm text-foreground transition-colors hover:bg-surface ${
                          c.code === selectedCountry ? 'bg-surface font-medium' : ''
                        }`}
                      >
      ...
      <section className="relative border-t border-border pb-4 lg:pb-6 overflow-hidden">
        <div className="pointer-events-none absolute inset-0 dev-grid-bg-fine opacity-[0.4] [mask-image:linear-gradient(to_bottom,transparent,black_20%,black_80%,transparent)]" />
          ...
          <div className="grid md:grid-cols-2 gap-4">
              ...
              <div className="rounded-xl border border-border bg-background p-6 md:p-8">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <a
                    ...
                    className="flex-shrink-0 rounded-md bg-foreground px-5 py-2.5 text-center text-sm font-medium text-background transition-colors cta-hover-gradient"
                  >
                    {signupLabel}
                  </a>
                </div>
                ...
                <button
                  type="button"
                  data-toggle-details
                  className="mt-4 flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground/80 cursor-pointer"
                >
                  See what's included
                ...
                {showDetails && (
                  <div className="mt-4">
                    <div className="mb-4">
                      <h3 className="text-sm font-semibold text-foreground mb-2">Coverage</h3>
                      <div className="flex items-center gap-2">
                        <Check className="h-4 w-4 text-primary" />
                        <span className="text-sm text-foreground/80">United States, India</span>
                      </div>
                    </div>
                    {(selectedCountry === "US" || selectedCountry === "IN") && (
                      <div className="mb-4">
                        <h3 className="text-sm font-semibold text-foreground mb-2">Channels included</h3>
                        <div className="space-y-2">
                          <div className="flex items-center gap-2">
                            <Check className="h-4 w-4 text-primary" />
                            <span className="text-sm text-foreground/80">Voice</span>
                          </div>
                          {selectedCountry === "US" && (
                            <div className="flex items-center gap-2">
                              <Check className="h-4 w-4 text-primary" />
                              <span className="text-sm text-foreground/80">SMS</span>
                            </div>
                          )}
                          {selectedCountry === "IN" && (
                            <>
                              <div className="flex items-center gap-2">
                                <Check className="h-4 w-4 text-primary" />
                                <span className="text-sm text-foreground/80">SMS (international routes only)</span>
                              </div>
                              <p className="text-xs text-muted-foreground pl-6">India domestic SMS is available on the Enterprise plan</p>
                            </>
                          )}
                        </div>
                      </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center gap-2">
                          <Check className="h-4 w-4 text-primary" />
                          <span className="text-sm text-foreground/80">2 RPS & 50 call concurrency</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <Check className="h-4 w-4 text-primary" />
                          <span className="text-sm text-foreground/80">Limited to {selectedCountry === "IN" ? "₹2,00,000" : "$2,500"} of monthly usage</span>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <div className="rounded-xl border border-border bg-background p-6 md:p-8">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <a
                    href="/contact/sales/"
                    className="flex-shrink-0 rounded-md border border-border-strong bg-background px-5 py-2.5 text-center text-sm font-medium text-foreground transition-colors hover:bg-surface"
                  >
                    Talk to sales
                  </a>
                </div>
                ...
                {showDetails && (
                  <div className="mt-4">
                    <div className="mb-4">
                      ...
                      <div className="flex items-center gap-2">
                        <Check className="h-4 w-4 text-primary" />
                        <span className="text-sm text-foreground/80">United States, India & 190 countries</span>
                      </div>
                    </div>
                    <div>
                      <h3 className="text-sm font-semibold text-foreground mb-2">Features</h3>
                      <p className="text-sm text-foreground/80 mb-2">Everything in Pay as you go and:</p>
                      <div className="space-y-2">
                        {selectedCountry === "IN" && (
                          <div className="flex items-center gap-2">
                            <Check className="h-4 w-4 text-primary" />
                            <span className="text-sm text-foreground/80">India domestic SMS</span>
                          </div>
                        )}
                        <div className="flex items-center gap-2">
                          <Check className="h-4 w-4 text-primary" />
                          <span className="text-sm text-foreground/80">Custom pricing & volume discounts</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <Check className="h-4 w-4 text-primary" />
                          <span className="text-sm text-foreground/80">Custom RPS & call concurrency</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <Check className="h-4 w-4 text-primary" />
                          <span className="text-sm text-foreground/80">Shortcodes & custom sender IDs</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <Check className="h-4 w-4 text-primary" />
                          <span className="text-sm text-foreground/80">Compliance - BAA, HIPAA & SSO</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <Check className="h-4 w-4 text-primary" />
                          <span className="text-sm text-foreground/80">Custom billing intervals</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <Check className="h-4 w-4 text-primary" />
                          <span className="text-sm text-foreground/80">Slack based onboarding</span>
                        </div>
                      </div>
                    </div>
                  </div>
      ...
      <section className="bg-background border-t border-border pb-12 lg:pb-20">
        <div className="container mx-auto max-w-7xl px-4">
          <h2 className="font-sans text-2xl font-semibold text-foreground mb-4 mt-5 text-center">Channel Pricing</h2>
          <div className="rounded-xl border border-border bg-background overflow-hidden">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 divide-y md:divide-y-0 md:divide-x divide-border">
              ...
              <div className="p-4 space-y-4 flex flex-col">
                ...
                <a href="/voice/pricing/" className="mt-auto inline-flex items-center gap-1 text-xs font-medium text-primary hover:underline pt-2">
              ...
              <div className="p-4 space-y-4 flex flex-col">
                <div className="flex items-center gap-2 py-2">
                  ...
                  <h3 className="text-base font-semibold text-foreground">SMS & RCS</h3>
                </div>
                <SMSPricingContent selectedCountry={selectedCountry} inline />
                <a href="/sms/pricing/" className="mt-auto inline-flex items-center gap-1 text-xs font-medium text-primary hover:underline pt-2">
                  View detailed pricing <span aria-hidden="true">&rarr;</span>
                </a>
              </div>
              ...
              <div className="p-4 space-y-4 flex flex-col">
                ...
                <a href="/whatsapp-message/pricing/" className="mt-auto inline-flex items-center gap-1 text-xs font-medium text-primary hover:underline pt-2">
                  View detailed pricing <span aria-hidden="true">&rarr;</span>
                </a>
              </div>
            </div>
            ...
            <div className="border-t border-border px-5 py-3">
              <p className="text-xs text-muted-foreground">All Voice and AI Agent calls are billed in 60-second intervals.</p>
            </div>
          </div>
        </div>
      </section>
